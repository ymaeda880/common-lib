...
EXCEL_PREVIEW_ROWS_PER_PAGE = 50
EXCEL_PREVIEW_MAX_COLS = 50
...
def load_xlsx_preview_df(
    ...
    *,
    ...
    start_row: int = 1,
    max_rows: int = EXCEL_PREVIEW_ROWS_PER_PAGE,
    max_cols: int = EXCEL_PREVIEW_MAX_COLS,
) -> Tuple[list[str], Optional[str], Optional[pd.DataFrame], int]:
    # ------------------------------------------------------------
    # xlsx のシート一覧を取得し、指定シートの一部を DataFrame 化する
    # ------------------------------------------------------------
    try:
        import openpyxl
    except Exception:
    ...
    try:
        ...
        for row in ws.iter_rows(
            min_row=max(1, int(start_row)),
            max_row=max(1, int(start_row)) + int(max_rows) - 1,
            min_col=1,
        ...
        return sheet_names, sheet_name, df, total_rows
...
def _render_excel_preview(
    *,
    ...
    # xlsx：シート選択 + 50行ページング
    # ------------------------------------------------------------
    sheet_names, default_sheet_name, _df_dummy, total_rows_dummy = load_xlsx_preview_df(
        file_path,
        sheet_name=None,
        start_row=1,
        max_rows=1,
    ...
    safe_excel_id = safe_preview_id(str(file_path))
    sheet_key = f"excel_preview_sheet__{safe_excel_id}"
    page_key = f"excel_preview_page__{safe_excel_id}"
    input_key = f"excel_page_input_{safe_excel_id}"
    ...
    # シートが変わった場合にページを1へ戻す
    last_sheet_key = f"excel_preview_last_sheet__{safe_excel_id}"
    if st.session_state.get(last_sheet_key) != selected_sheet:
        st.session_state[last_sheet_key] = selected_sheet
        st.session_state[page_key] = 1

    sheet_names, sheet_name, _df_dummy, total_rows = load_xlsx_preview_df(
        file_path,
    ...
    rows_per_page = EXCEL_PREVIEW_ROWS_PER_PAGE
    total_pages = max(1, int(math.ceil(max(1, total_rows) / rows_per_page)))
    ...
    cur_page = clamp_page_no(cur_page, total_pages)
    st.session_state[page_key] = cur_page

    c_prev, c_next, c_page, c_info = st.columns([1, 1, 1.2, 4])

    with c_prev:
        if st.button("⬅ 前へ", disabled=(cur_page <= 1), key=f"excel_prev_{safe_excel_id}"):
    ...
    with c_next:
        if st.button("次へ ➡", disabled=(cur_page >= total_pages), key=f"excel_next_{safe_excel_id}"):
    ...
    if input_key not in st.session_state:
        st.session_state[input_key] = cur_page
    ...
    if int(page_input) != int(st.session_state[page_key]):
        st.session_state[page_key] = int(page_input)
        st.rerun()

    start_row = (int(st.session_state[page_key]) - 1) * rows_per_page + 1

    _, sheet_name, df_prev, total_rows = load_xlsx_preview_df(
        file_path,
        sheet_name=selected_sheet,
        start_row=start_row,
    ...
    with c_info:
        st.caption(
            f"シート: {sheet_name}　"
            f"行 {start_row}–{end_row} / {total_rows}　"
            f"ページ {st.session_state[page_key]} / {total_pages}　"
            f"最大 {EXCEL_PREVIEW_MAX_COLS} 列表示"
        )
