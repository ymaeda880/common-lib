...
def transcribe_http(
    *,
    ...
    filename: str,
    mime_type: str,
    response_format: str = "json",  # json/text/srt/vtt/diarized_json
    ...
    # - diarizationモデルではchunking_strategyが必須
    # ============================================================
    ...
    try:
        ...
        raise RetryableError(
            "OpenAI transcribe timeout",
            provider="openai",
        ) from e

    except Exception as e:
        raise ProviderError(
            f"OpenAI transcribe request failed: {e}",
            provider="openai",
        ) from e

    req_id = resp.headers.get(
        "x-request-id"
    )
    ...
    if response_format in (
        ...
        try:
            ...
            raise InvalidResponseError(
                f"OpenAI transcribe: JSON解析に失敗しました: {e}"
            ) from e
